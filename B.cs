...
static Nullable<_.Token> token ;
static bool debug ;
static void jump_( ref xyzzyy b )
	{
	...
	if( state.default_item.HasValue ) 
		...
		Reduction r ;
		i = state.itemset[state.default_item.Value] ;
		reduction( _default, out r ) ;
		state.zlog.Add( i ) ;
		state.zlog.Add( r ) ;
		if( r.item.rule != i.rule || r.item.point != i.point )
			Console.Beep() ;
		if( state.lookaheadset.Count == 0 )
			{
			throw new ReducedAcception( i.rule ) ;
			}
		else
		if( state.lookaheadset.Contains( b.yy )  )
			{
			state.zlog.Add( "LOOKAHEAD" ) ;
			throw new ReducedAcception( i.rule ) ; 
			}
		}
