		...
		if( state.transitionset.Length > 0 )
			...
			foreach( Transition t in state.transitionset )
				if( t == b.yy )
					...
					state.zlog.Add( b.yy ) ;
					state.zlog.Add( t ) ;
					xyzzy = new xyzzyy( t.item.rule, t.item.point, t.state, (int)_default ) ; //_FIX:jo{t`_x.y}[^_`']{{_entity[':']}}[,_s[,_bit[-bit]]][^_`'] //"bottoms-out" on contextual deduction between locked states and convexion
					goto _jump ;
					}
			}
		state.zlog.Add( b.yy ) ;
